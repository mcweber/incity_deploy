...
class LLMHandler:
    ...
    def __init__(self, llm: str = "gemini", local: bool = False):
        ...
        if self.LLM == "gpt4o" or self.LLM == "gpt4omini":
            self.openaiClient = OpenAI(api_key=os.environ.get('OPENAI_API_KEY_PRIVAT'))
        elif self.LLM == "llama":
            self.groqClient = Groq(api_key=os.environ.get('GROQ_API_KEY_PRIVAT'))
        elif self.LLM == "gemini":
            self.geminiClient = OpenAI(
    ...
    def _handle_remote_llm(self, temperature: float, input_messages: list) -> str:
        if self.LLM == "gpt4o":
            ...
            response = self.geminiClient.chat.completions.create(model="gemini-1.5-flash-latest", messages=input_messages)
            return(response.choices[0].message.content)
        else:
