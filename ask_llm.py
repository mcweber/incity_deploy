...
# Author: M. Weber
# ---------------------------------------------------
# 13.09.2025 switched to gpt4
# 13.09.2025 eliminated temperature parameter
# 04.11.2025 switched to gpt-4-turbo amd gpt-4o
# 20.11.2025 switched gpt-5.1
# ---------------------------------------------------
# Description:
# llm: gemini, gpt-5.1, deepseek, llama
# local: True/False
# ---------------------------------------------------
...
# from modules.time_wrapper import timer

MODELS = ["gemini", "gpt-5.1", "deepseek", "llama"]
...
class LLMHandler:
    def __init__(self, llm: str = "gemini", local: bool = False):
        ...
        if self.LLM in ["gpt-5.1"]:
            self.openaiClient = openai.OpenAI(
                api_key=os.environ.get("OPENAI_API_KEY_DVV")
            )
        elif self.LLM == "llama":
    ...
    def define_prompt(
        ...
        if db_results_str:
            prompt.append(
                {
                    "role": "assistant",
                    "content": f"Hier sind einige relevante Informationen aus der Datenbank:\n{db_results_str}",
                }
            )
        if web_results_str:
            prompt.append(
                {
                    "role": "assistant",
                    "content": f"Hier sind einige relevante Informationen aus dem Internet:\n{web_results_str}",
                }
            )
        if source_doc_str:
            prompt.append(
                {
                    "role": "assistant",
                    "content": f"Dies ist das Quelldokument:\n{source_doc_str}",
                }
            )
        question_prefix = (
            "Basierend auf den beigefügten Informationen, "
            if web_results_str or db_results_str
            else ""
        )
    ...
    def _handle_remote_llm(self, input_messages: list) -> str:
        ...
        if self.LLM in ["gpt-5.1"]:
            response = self.openaiClient.chat.completions.create(
                model=self.LLM, messages=input_messages
            )
            return response.choices[0].message.content
        elif self.LLM == "llama":
            response = self.groqClient.chat.completions.create(
                model="llama-3.3-70b-versatile", messages=input_messages
            )
            return response.choices[0].message.content
        elif self.LLM == "gemini":
            response = self.geminiClient.chat.completions.create(
                model="gemini-2.0-flash", messages=input_messages
            )
            return response.choices[0].message.content
        elif self.LLM == "deepseek":
            response = self.deepseekClient.chat.completions.create(
                model="deepseek-chat", messages=input_messages
            )
            return response.choices[0].message.content
        else:
